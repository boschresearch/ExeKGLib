...
3. [❗ **This step is optional**: Perform only if the new method cannot be "thematically associated" with an existing sub-class of `ds:AtomicTask`]
    ...
    1. Add a new sub-class of [ds:AtomicTask](https://nsai-uio.github.io/ExeKGOntology/OnToology/ds_exeKGOntology.ttl/documentation/index-en.html#https://raw.githubusercontent.com/nsai-uio/ExeKGOntology/main/ds_exeKGOntology.ttl#AtomicTask).
        ...
        ml:{NewTask}
            a               owl:Class ;
            rdfs:subClassOf ml:{UpperTaskClass},    # this relationship is ONLY for *Case 2*
                            ds:AtomicTask .
        ```
        **In the above template**:
        - 🗒️ **Note**: If ✳️**Case 2** applies, then the relationship `rdfs:subClassOf ml:{UpperTaskClass}` is not needed.
        - `{NewTask}` should be replaced with the new task's name in **camel-case**.
    ...
    2. Add a new property that will connect the new task with the new method.
        ...
        **In the above template**:
        ...
        - `{NewMethod}` should be replaced with the new method's name in **camel-case** (see **next Steps** for how to **create a new method**). <br>✍️ For **example**, if the new task belongs under the `ml:Classification` task which belongs under the `ml:Train` task, then `{UpperTaskToMethodProperty}` should be `hasTrainMethod`.
    3. Add a new SHACL shape for task-to-method link in `ml_shacl_shapes.ttl` (replace `ml` with the chosen schema's namespace prefix).
        ```turtle
    ...
    4. *TODO*: add instructions for adding SHACL shapes about method parameters
    5. [🗒️ **Note**: Perform this step only if ✳️**Case 2** applies. For existing tasks, the inputs and outputs for each task are defined in the [bottom-level KG schemata](https://github.com/boschresearch/ExeKGLib#kg-schemata) as inputs and outputs of the top-level task classes. ✍️ For **example**, any task that belongs under the `ml:Train` task, will receive two inputs `ml:DataInTrainX` and `ml:DataInTrainY`, and produce an output `ml:DataOutTrainModel`.]
        ...
        **In the above template**:
        ...
        - `{NewTask}` should be replaced with the new task's name from Step 3.a.
        - `{Input1}`, ..., `{InputN}` and `{Output1}`, ..., `{OutputN}` should be replaced with **unique** input and output names, respectively.
    ...
    6. [🗒️ **Note**: Perform this step only if ✳️**Case 2** applies. For existing tasks, the input and output SHACL shapes for each task are defined in the bottom-level SHACL shape graphs (e.g. `ml_shacl_shapes.ttl`) as input and output SHACL shapes of the top-level task classes. ✍️ For **example**, for any task that belongs under the `ml:Train` task, the corresponding shape is `:TrainTaskInputOutputShape`]
...
5. Open `generated_schemata/generated_{prefix}_ontologies.ttl` after replacing `{prefix}` with the namespace prefix of the desired bottom-level KG schema. `ml` for Machine Learning, `stats` for Statistics and `visu` for Visualization.
6. Add a new sub-class of [ds:AtomicMethod](https://nsai-uio.github.io/ExeKGOntology/OnToology/ds_exeKGOntology.ttl/documentation/index-en.html#https://raw.githubusercontent.com/nsai-uio/ExeKGOntology/main/ds_exeKGOntology.ttl#AtomicMethod) together with its Python module hierarchy.
    ```turtle
    ml:{NewMethod}Method
        a               owl:Class ;
    ...
    ```
    **In the above template**:
    - `{NewMethod}` should be replaced with the new method's name in **camel-case**.
...
7. [🗒️ **Note**: If the optional Step 3 was performed, this step can be skipped]
    ...
    2. In the set of existing values of `rdfs:range` property, add `ml:{NewMethod}Method`. Here, `{NewMethod}` should be replaced with the new method's name from Step 5.
    ...
    4. In the set of existing values of `sh:or` that is under `sh:property`, add `[ sh:class ml:{NewMethod}Method ]`. Here, `{NewMethod}` should be replaced with the new method's name from Step 5.
8. Add the desired parameters as data properties for the new method.
    ```turtle
    ...
    **In the above template**:
    ...
    - `{NewParam1}` and `{NewParam2}` should be replaced with the desired parameter names in **camel-case**.
    - `{NewMethod}` should be replaced with the new method's name from Step 5.
    - `{Range1}` and `{Range2}` should be replaced with the desired literal value ranges (e.g. `xsd:float`).

9.  Modify `config.py` in `exe_kg_lib` package to update the value of `KG_SCHEMAS_DIR` to point to the cloned repo's directory from Step 1.
...
1. Find the **relevant sub-class** of `exe_kg_lib.classes.task.Task` in the **corresponding file** of the `exe_kg_lib.classes.tasks` package.
    ...
    - The **relevant sub-class** depends on the task to which the new method was linked in Step 3.b or Step 6 of [Section A of this guide](#a-adding-semantic-components-to-a-bottom-level-kg-schema-and-shacl-shapes-graph). <br>✍️ For **example**, if the new method is linked to the `ml:BinaryClassification` task which belongs under the `ml:Train` task, then the **relevant sub-class** is `exe_kg_lib.classes.tasks.ml_tasks.Train`.<br>
    [🗒️ **Note**: If ✳️**Case 2 from Step 3** applies, then the **relevant sub-class** refers to a new Python class that needs to be created in a similar way to existing classes in `exe_kg_lib.classes.tasks` package.]
2. Modify the code of `run_method()` in the **relevant sub-class** to call the Python module that implements the new method (see Step 5 in [Section A of this guide](#a-adding-semantic-components-to-a-bottom-level-kg-schema-and-shacl-shapes-graph)).
